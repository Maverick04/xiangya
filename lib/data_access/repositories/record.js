...
module.exports.removeById = function(recordid){
   ...
   dataModels.Record.findByIdAndRemove(recordid,function(err,res){
      if(err){
          ...
          dataModels.Entity.find({"_id":{$in:res.details}}).remove(function(err,res){
              if(err){
                  def.reject(err);
              } else if(_.isNull(res)){
                  def.reject(new exceptions.ResourceNotFoundException("Fail to remove entities"));
              } else {
          	  def.resolve(true);
              }
          });
...
module.exports.getByUser = function(userid){
   ...
   if(_.isNull(userid) || _.isUndefined(userid)){
       def.reject(exceptions.ArgumentException('Invalid parameter: userid'));
       return def.promise;
   }
   ...
   dataModels.User.findById(userid,{'_id':0,'patients':1}, function(err,res){
       if(err){
           ...
           dataModels.Record.find({'patientId':{$in:res.patients}},{'_id':1,'patientId':1,'departmentId':1,'keyword':1,'createDate':1},function(err,res){
              if(err){
                  def.reject(err);
              } else if(_.isNull(res)){
                  def.reject(new exceptions.ResourceNotFoundException('No Record With Id: ' + recordid));
