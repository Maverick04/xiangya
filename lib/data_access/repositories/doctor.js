 ...
 *return {Promise} doctor object
 */
module.exports.getByDepartment =function(departmentid){
    var def = q.defer();
    ...
    dataModels.Doctor.find({"departmentId":departmentid},{'name':1,'introduction':1,'title':1},function(err,res){
    	if(err){
            def.reject(err);
        } else if (_.isNull(res)){
            def.reject(new exceptions.ResourceNotFoundException("No doctor with departmentid: " + departmentid));
        } else {
            def.resolve(res);
        }
   });
